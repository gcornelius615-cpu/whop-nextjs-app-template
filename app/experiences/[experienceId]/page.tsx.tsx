...
interface Leg {
  p: string; g: string; s: string; o: string; c: number; lock: boolean; result: string;
}
interface CardColors {
  a: string; b: string; s: string; f: string;
}
...
const VIDEO_FMTS = [
  {id:"9x16",n:"Vertical",sz:"1080×1920",pl:"TikTok · Reels · Shorts",w:432,h:768,r:20},
  {id:"4x5", n:"Portrait", sz:"1080×1350",pl:"Instagram Feed",w:432,h:540,r:16},
  {id:"1x1", n:"Square",   sz:"1080×1080",pl:"Instagram · Twitter",w:480,h:480,r:16},
];
const IMAGE_FMTS = [
  {id:"9x16i",n:"Vertical", sz:"1080×1920",pl:"TikTok · IG Story",w:432,h:768,r:20},
  {id:"1x1i", n:"Square",   sz:"1080×1080",pl:"Instagram · Twitter",w:480,h:480,r:16},
  {id:"4x5i", n:"Portrait", sz:"1080×1350",pl:"Instagram Feed",w:432,h:540,r:16},
  {id:"16x9", n:"Landscape",sz:"1280×720", pl:"YouTube · Twitter",w:640,h:360,r:12},
  {id:"2x3",  n:"Pinterest",sz:"1000×1500",pl:"Pinterest Pin",w:360,h:540,r:14},
];
...
const GRADS = [
  {n:"None",    g:null},
  {n:"Sunset",  g:"linear-gradient(135deg,#FF6B35,#FFD700)"},
  {n:"Aurora",  g:"linear-gradient(135deg,#38BDF8,#A855F7)"},
  {n:"Toxic",   g:"linear-gradient(135deg,#C6FF3A,#06B6D4)"},
  {n:"Blaze",   g:"linear-gradient(135deg,#FF3B3B,#FF6B35)"},
  {n:"Candy",   g:"linear-gradient(135deg,#F472B6,#A855F7)"},
  {n:"Money",   g:"linear-gradient(135deg,#4ADE80,#FFD700)"},
  {n:"Ocean",   g:"linear-gradient(135deg,#06B6D4,#818CF8)"},
  {n:"Inferno", g:"linear-gradient(135deg,#FF0000,#FF6B35,#FFD700)"},
  {n:"Mint",    g:"linear-gradient(135deg,#00F5A0,#00D9F5)"},
];

const ANIMS = [
  {id:"smooth",l:"Smooth",d:220,t:0.38},
  {id:"fast",  l:"Fast",  d:80, t:0.15},
  {id:"cinema",l:"Cinema",d:420,t:0.6},
  {id:"instant",l:"Instant",d:0,t:0.01},
];
...
const LAYOUTS = [
  {id:"ly-stacked",l:"Stacked"},{id:"ly-compact",l:"Compact"},
  {id:"ly-bold",l:"Bold"},{id:"ly-minimal",l:"Minimal"},
];

const FONTS = [
  {id:"athletic",  l:"Athletic",    h:"Anton",        b:"Oswald"},
  {id:"streetwear",l:"Streetwear",  h:"Bebas Neue",   b:"Barlow Condensed"},
  {id:"modern",    l:"Modern",      h:"Inter",        b:"Space Grotesk"},
];
...
function fmtOdds(raw: string, fmt: string): string {
  ...
  if (fmt === "fractional") {
    const d = toDecimal(raw) - 1;
    if (d <= 0) return raw;
    const gcd = (a: number, b: number): number => b ? gcd(b, a % b) : a;
    const n = Math.round(d * 100), dn = 100, g = gcd(n, dn);
    return `${n/g}/${dn/g}`;
  }
  return raw;
}
...
function Btn({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) {
  return (
    <button onClick={onClick} style={{
      padding:"7px 13px",
      background:active?"color-mix(in srgb, var(--pacc) 8%, transparent)":"#0d0f14",
      border:`1px solid ${active?"var(--pacc)":"rgba(255,255,255,.07)"}`,
      borderRadius:7, color:active?"var(--pacc)":"#4a5060",
...
function Fld({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:3 }}>
      <label style={{ fontSize:9, color:"#4a5060", letterSpacing:1, textTransform:"uppercase", display:"block", marginBottom:3 }}>{label}</label>
      {children}
    </div>
  );
}
...
function Sec({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div style={{ background:"#15181e", border:"1px solid rgba(255,255,255,.055)", borderRadius:13, padding:"16px 18px" }}>
      <div style={{ fontSize:9, letterSpacing:3, color:"#4a5060", textTransform:"uppercase", marginBottom:12, display:"flex", alignItems:"center", gap:8 }}>
        {title}
...
export default function ParlayBuilder() {
  ...
  const [currentLayout, setCurrentLayout] = useState("ly-stacked");
  ...
  const compute = useCallback(() => {
    if (betType === "single") {
      const dec = toDecimal(sbOdds);
      const profit = (stake * dec) - stake;
    ...
    let dec = 1;
    legs.forEach(l => { dec *= toDecimal(l.o); });
    const profit = (stake * dec) - stake;
    const total = stake * dec;
    const payVal = payoutMode === "total" ? total : profit;
    const am = dec >= 2 ? `+${Math.round((dec-1)*100)}` : `${Math.round(-100/(dec-1))}`;
  ...
  const applyTheme = useCallback((colors: CardColors) => {
    ...
    st.style.setProperty("--muted", "#5a6070");
  ...
  const applyFormat = useCallback((fmt: typeof VIDEO_FMTS[0]) => {
    ...
    const st = stageRef.current;
    if (!st) return;
    st.style.width = `${fmt.w}px`;
    st.style.height = `${fmt.h}px`;
    st.style.borderRadius = `${fmt.r}px`;
    const frame = st.querySelector<HTMLElement>("#s-frame");
    if (frame) {
      const pH = Math.round(fmt.h * .034), pW = Math.round(fmt.w * .051);
      frame.style.padding = `${pH}px ${pW}px ${Math.round(pH*.7)}px`;
    }
  }, []);
  ...
  useEffect(() => {
    const base = stageRef.current?.querySelector<HTMLElement>("#s-base");
    if (!base) return;
    if (bgImgSrc) {
  ...
  useEffect(() => {
    const st = stageRef.current;
    if (!st) return;
    const bl = st.querySelector<HTMLElement>("#s-brd");
    if (!bl) return;
    bl.style.boxShadow = "";
  ...
  useEffect(() => {
    ...
    if (!stage) return;
    const allText = stage.querySelectorAll<HTMLElement>(".cc-brand,.cc-type,.cc-title,.cc-date,.cc-rec,.cc-sub,.cc-hash,.l-pick,.l-meta,.cc-slbl,.cc-btag span");
    const titleEls = stage.querySelectorAll<HTMLElement>(".cc-title,.cc-brand,.l-odds,.cc-sval");
    allText.forEach(el => { el.style.fontFamily = `'${currentFont.b}', sans-serif`; });
    titleEls.forEach(el => { el.style.fontFamily = `'${currentFont.h}', sans-serif`; });
  }, [currentFont]);

  useEffect(() => {
    const legsEl = stageRef.current?.querySelector<HTMLElement>("#e-legs");
    if (!legsEl) return;
    ["ly-stacked","ly-compact","ly-bold","ly-minimal"].forEach(c => legsEl.classList.remove(c));
    legsEl.classList.add(currentLayout);
  }, [currentLayout]);
  ...
  const playAnim = useCallback(() => {
    ...
    const total = 150 + (legEls.length * cfg.d) + 400;
    legEls.forEach((l, i) => {
      l.style.transition = `opacity ${cfg.t}s, transform ${cfg.t}s`;
      const t = setTimeout(() => {
        l.classList.add("vis");
  ...
  }, [animId, useConf, useLoop]);

  const fireConfetti = useCallback(() => {
    const layer = stageRef.current?.querySelector<HTMLElement>("#s-conf");
    if (!layer) return;
    layer.innerHTML = "";
  ...
  useEffect(() => {
    const t = setTimeout(() => startPlay(), 700);
  ...
  const loadH2C = (): Promise<(el: HTMLElement, opts: object) => Promise<HTMLCanvasElement>> => {
  ...
  const doDownload = async () => {
    ...
    try {
      stageRef.current?.querySelectorAll<HTMLElement>("#e-legs .leg").forEach(l => {
        l.classList.remove("vis"); l.style.transition = "none";
      });
      const bar = stageRef.current?.querySelector<HTMLElement>("#e-prog");
      if (bar) bar.style.width = "0%";
      await new Promise(r => setTimeout(r, 80));

      const W = currentFmt.w * 2.5;
      const H = currentFmt.h * 2.5;
      ...
      rec.onstop = () => {
        ...
        const a = document.createElement("a");
        a.download = `postlabs-picks-${currentFmt.id}.webm`;
      ...
      const cfg = ANIMS.find(a => a.id === animId) || ANIMS[0];
      const duration = 200 + (legCount * cfg.d) + 1400;

      const frameInterval = setInterval(async () => {
        try {
          const snap = await h2c(stageRef.current!, {
            scale: 2.5, useCORS: true, allowTaint: true,
            backgroundColor: null, logging: false,
          });
      ...
      setDlMsg("Video recording failed. Try PNG instead.");
      console.error(e);
      setDlBusy(false);
      setTimeout(() => setDlMsg(""), 4000);
    }
  ...
  const renderLegs = () => {
    if (betType === "single") {
      ...
      return (
        <div className="leg vis" style={{ padding:"14px", borderRadius:12, gap:12, opacity:1, transform:"none" }}>
          <div className="l-icon" style={{ width:40, height:40, fontSize:22, borderRadius:10 }}>{sp.e}</div>
    ...
    <div style={{ background:"#0c0e12", fontFamily:"Oswald, sans-serif", color:"#f0f2f0", minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", padding:"24px 12px 100px", gap:24 }}>
      ...
      <div style={{ width:"100%", maxWidth:1040, display:"flex", alignItems:"center", justifyContent:"space-between", paddingBottom:12, borderBottom:"1px solid rgba(255,255,255,.06)" }}>
        <div>
          ...
          <span style={{ fontFamily:"Oswald, sans-serif", fontSize:11, color:"#4a5060", letterSpacing:3, marginLeft:10, textTransform:"uppercase" }}>Picks</span>
        </div>
        <div style={{ fontSize:9, color:"#2a2f3a", letterSpacing:2, textTransform:"uppercase" }}>v1.0 · PostLabs</div>
      </div>
      ...
      <div style={{ display:"flex", gap:24, alignItems:"flex-start", width:"100%", maxWidth:1040, flexWrap:"wrap", justifyContent:"center" }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:10, flexShrink:0 }}>
          <div style={{ fontSize:9, letterSpacing:3, color:"#4a5060", textTransform:"uppercase", textAlign:"center" }}>{currentFmt.pl} — {currentFmt.sz}</div>
          <div id="stage" ref={stageRef} style={{
            ...
            ["--acc" as string]:C.a, ["--bg" as string]:C.b, ["--card2" as string]:C.s,
            ["--ink" as string]:C.f, ["--muted" as string]:"#5a6070", ["--line" as string]:"rgba(255,255,255,.09)",
          }}>
            <div id="s-base" style={{ position:"absolute",inset:0,zIndex:0,backgroundColor:C.b }} />
            <div id="s-over" style={{ position:"absolute",inset:0,zIndex:1,pointerEvents:"none" }} />
            <div id="s-pat"  style={{ position:"absolute",inset:0,zIndex:2,pointerEvents:"none",display:"none" }} />
            <div id="s-grain" style={{ position:"absolute",inset:0,zIndex:3,pointerEvents:"none",display:"none",mixBlendMode:"overlay",backgroundImage:`url("data:image/svg+xml,%3Csvg viewBox='0 0 200 200' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='n'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.85' numOctaves='4'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23n)' opacity='0.5'/%3E%3C/svg%3E")`,backgroundSize:"200px 200px" }} />
            <div id="s-brd"  style={{ position:"absolute",inset:0,zIndex:5,pointerEvents:"none",borderRadius:"inherit" }} />
            <div id="s-conf" style={{ position:"absolute",inset:0,zIndex:19,pointerEvents:"none",overflow:"hidden" }} />
            <div id="s-cd" style={{ position:"absolute",inset:0,zIndex:20,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(0,0,0,.75)",opacity:0,pointerEvents:"none",transition:"opacity .3s" }}>
              <div id="s-cdn" style={{ fontFamily:"Anton,sans-serif",fontSize:140,color:"var(--acc)",lineHeight:1,transform:"scale(1.5)",opacity:0,transition:"transform .2s,opacity .2s" }}>3</div>
            </div>
            <div id="s-frame" style={{ position:"absolute",inset:0,display:"flex",flexDirection:"column",padding:"26px 22px 18px",zIndex:6 }}>
              <div className="cc-head">
                ...
                <div className="cc-hr">
                  ...
                  {logoSrc && <img className="cc-logo" src={logoSrc} alt="" style={{ display:"block" }} />}
                </div>
              </div>
              ...
              <div className={`cc-legs ${currentLayout}`} id="e-legs">{renderLegs()}</div>
              <div className="cc-prog-wrap"><div className="cc-prog" id="e-prog" style={{ width:`${progress}%` }} /></div>
        ...
        <div style={panelStyle}>
          ...
          <Sec title="Output Type">
            <div style={{ display:"flex", background:"#0d0f14", border:"1px solid rgba(255,255,255,.08)", borderRadius:9, overflow:"hidden", marginBottom:14 }}>
              {["video","image"].map(m => (
                <button key={m} onClick={() => { setOutputMode(m as "video"|"image"); if(m==="image") startPlay(); }}
                  style={{ flex:1,padding:10,fontFamily:"Oswald,sans-serif",fontSize:11,letterSpacing:2,textTransform:"uppercase",border:"none",background:outputMode===m?C.a:"transparent",color:outputMode===m?"#06080B":"#4a5060",cursor:"pointer",transition:"all .2s" }}>
                  {m==="video"?"▶ Video":"⬜ Static Image"}
                </button>
              ))}
            </div>
            <div style={{ fontSize:9,letterSpacing:3,color:"#4a5060",textTransform:"uppercase",marginBottom:8 }}>Platform & Format</div>
            <div style={{ display:"flex",gap:8,flexWrap:"wrap" }}>
              {(outputMode==="video"?VIDEO_FMTS:IMAGE_FMTS).map(f => (
                <button key={f.id} onClick={() => applyFormat(f)}
                  style={{ background:currentFmt.id===f.id?`color-mix(in srgb,${C.a} 8%,transparent)`:"#0d0f14",border:`1px solid ${currentFmt.id===f.id?C.a:"rgba(255,255,255,.07)"}`,borderRadius:9,padding:"9px 13px",cursor:"pointer",display:"flex",flexDirection:"column",gap:2,textAlign:"left",transition:"all .15s" }}>
                  ...
                  <span style={{ fontSize:8,color:"#4a5060" }}>{f.sz}</span>
                  <span style={{ fontSize:7,color:"#3a3f4a",marginTop:1 }}>{f.pl}</span>
                </button>
              ))}
            </div>
          </Sec>

          <Sec title="Quick Presets">
            ...
            <div style={{ fontSize:9,letterSpacing:3,color:"#4a5060",textTransform:"uppercase",marginBottom:8 }}>Custom Colors</div>
            <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:9,marginBottom:10 }}>
              {(["a","b","s","f"] as const).map((ch,i) => (
                <div key={ch} style={{ background:"#0d0f14",border:"1px solid rgba(255,255,255,.08)",borderRadius:9,padding:"9px 11px" }}>
                  <label style={{ fontSize:8,color:"#4a5060",letterSpacing:1.5,textTransform:"uppercase",display:"block",marginBottom:5 }}>{["Accent","Background","Surface","Font"][i]}</label>
            ...
            {recentAccents.length > 0 && (
              <div style={{ display:"flex",gap:5,alignItems:"center",marginTop:8 }}>
                <span style={{ fontSize:8,color:"#3a3f4a",letterSpacing:1,textTransform:"uppercase",flexShrink:0 }}>Recent:</span>
            ...
            <div style={{ fontSize:9,letterSpacing:3,color:"#4a5060",textTransform:"uppercase",margin:"14px 0 8px" }}>Gradient Accents</div>
            <div style={{ display:"flex",gap:7,flexWrap:"wrap" }}>
              {GRADS.map(g => (
                <div key={g.n} onClick={() => { if(g.g) stageRef.current?.style.setProperty("--acc",g.g); else applyTheme(C); }} title={g.n}
                  style={{ width:44,height:26,borderRadius:7,cursor:"pointer",background:g.g||"#1a1d23",border:"2px solid transparent",flexShrink:0 }} />
              ))}
            </div>
          </Sec>
          ...
          <Sec title="Background Image">
            <div style={{ border:"1px dashed rgba(255,255,255,.12)",borderRadius:9,padding:12,textAlign:"center",cursor:"pointer",position:"relative",overflow:"hidden",marginBottom:10 }}>
              ...
              <p style={{ fontSize:10,letterSpacing:2,color:"#4a5060",textTransform:"uppercase" }}>Drop image or click</p>
              <small style={{ fontSize:9,color:"#2a2f3a" }}>JPG · PNG · WebP</small>
            </div>
            <label style={{ fontSize:9,color:"#4a5060",letterSpacing:1,textTransform:"uppercase" }}>Opacity</label>
            <input type="range" min={5} max={100} value={bgImgOp} onChange={e => setBgImgOp(Number(e.target.value))} style={{ width:"100%",marginTop:4 }} />
            {bgImgSrc && <button onClick={() => setBgImgSrc(null)} style={{ marginTop:8,width:"100%",padding:"7px 13px",background:"#0d0f14",border:"1px solid rgba(255,255,255,.07)",borderRadius:7,color:"#4a5060",fontFamily:"Oswald,sans-serif",fontSize:10,letterSpacing:1,textTransform:"uppercase",cursor:"pointer" }}>✕ Remove Image</button>}
          </Sec>

          <Sec title="Pattern Overlay">
            <div style={{ display:"flex",gap:7,flexWrap:"wrap",marginBottom:14 }}>
              {PATS.map(p => <Btn key={p.id} label={p.l} active={currentPatId===p.id} onClick={()=>setCurrentPatId(p.id)} />)}
            </div>
            <div style={{ fontSize:9,letterSpacing:3,color:"#4a5060",textTransform:"uppercase",marginBottom:8 }}>Card Border</div>
            <div style={{ display:"flex",gap:7,flexWrap:"wrap" }}>
              {BRDS.map(b => <Btn key={b.id} label={b.l} active={currentBrdId===b.id} onClick={()=>setCurrentBrdId(b.id)} />)}
            </div>
          </Sec>

          <Sec title="Card Style">
            <div style={{ fontSize:9,letterSpacing:3,color:"#4a5060",textTransform:"uppercase",marginBottom:8 }}>Typography</div>
            <div style={{ display:"flex",gap:7,flexWrap:"wrap",marginBottom:14 }}>
              {FONTS.map(f => <Btn key={f.id} label={f.l} active={currentFont.id===f.id} onClick={()=>setCurrentFont(f)} />)}
            </div>
            <div style={{ fontSize:9,letterSpacing:3,color:"#4a5060",textTransform:"uppercase",marginBottom:8 }}>Layout</div>
            <div style={{ display:"flex",gap:7,flexWrap:"wrap" }}>
              {LAYOUTS.map(l => <Btn key={l.id} label={l.l} active={currentLayout===l.id} onClick={()=>setCurrentLayout(l.id)} />)}
            </div>
          </Sec>
          ...
          <Sec title="Brand & Identity">
            ...
            <Fld label="Page Logo">
              <div style={{ border:"1px dashed rgba(255,255,255,.12)",borderRadius:9,padding:12,textAlign:"center",cursor:"pointer",position:"relative",overflow:"hidden" }}>
                ...
                <p style={{ fontSize:10,letterSpacing:2,color:"#4a5060",textTransform:"uppercase" }}>{logoSrc?"Logo uploaded ✓":"Drop logo or click"}</p>
                <small style={{ fontSize:9,color:"#2a2f3a" }}>Transparent PNG</small>
              </div>
            </Fld>
          </Sec>
          ...
          <Sec title="Bet Type">
            <div style={{ display:"flex",gap:7,marginBottom:14 }}>
              <Btn label="Parlay" active={betType==="parlay"} onClick={()=>setBetType("parlay")} />
              <Btn label="Single Bet" active={betType==="single"} onClick={()=>setBetType("single")} />
            </div>
            {betType==="parlay" && <>
              <div style={{ fontSize:9,letterSpacing:3,color:"#4a5060",textTransform:"uppercase",marginBottom:8 }}>Legs</div>
              <div style={{ display:"flex",gap:7,flexWrap:"wrap",marginBottom:12 }}>
                {[2,3,4,5,6,7,8].map(n => <Btn key={n} label={`${n} Legs`} active={legCount===n} onClick={()=>setLegCount(n)} />)}
              </div>
              ...
              <div style={{ display:"grid",gridTemplateColumns:"155px 1fr 1fr 66px 50px 70px 68px",gap:6,marginBottom:5 }}>
                {["Sport","Pick","Game / Market","Odds","Conf","Lock","Result"].map(h=>(
                  <span key={h} style={{ fontSize:8,color:"#2a2f3a",letterSpacing:1,textTransform:"uppercase" }}>{h}</span>
                ))}
              </div>
              {legs.map((l,i) => (
          ...
          <Sec title="Animation (video only)">
            <div style={{ display:"flex",gap:7,flexWrap:"wrap",marginBottom:10 }}>
              {ANIMS.map(a=><Btn key={a.id} label={a.l} active={animId===a.id} onClick={()=>setAnimId(a.id)} />)}
            </div>
          ...
          <button onClick={doDownload} disabled={dlBusy} style={{ width:"100%",padding:13,background:"transparent",border:`2px solid ${C.a}`,borderRadius:9,color:C.a,fontFamily:"Anton,sans-serif",fontSize:14,letterSpacing:2,cursor:"pointer",textTransform:"uppercase",marginTop:2,opacity:dlBusy?0.5:1 }}>
            {dlBusy?"Processing...":`⬇ Download ${outputMode==="image"?"PNG":"Video"}`}
          </button>
          {dlMsg && <div style={{ fontSize:9,color:"#4a5060",textAlign:"center",letterSpacing:1,textTransform:"uppercase" }}>{dlMsg}</div>}
          <p style={{ fontSize:9,color:"#2a2f3a",textAlign:"center",letterSpacing:1,textTransform:"uppercase" }}>Preview Mode hides controls for a clean full-card view</p>
